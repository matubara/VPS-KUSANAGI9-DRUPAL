...
if [ $# -eq 0 ];then
	...
	constname=const_deploydrupalonconohavps.v100.sh
    echo "引数が設定されていないのでデフォルトの設定ファイルを読み込みます"
    read -p $CONFIRMMES
fi


    echo "設定ファイルは、 ${constname} です。"
...
echo "kusanagi provision --lamp --dbname=${dbname} --dbuser=${dbuser} --dbpass=${dbpass} --email=${email} --fqdn ${FQDN} ${kusanagiprofile}"
